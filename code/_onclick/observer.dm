...
/mob/dead/observer/verb/ascend()
	...
	if(src.client.wraith_pain || locate(/obj/effect/landmark/dungeonmap))
		...
		M.key = key
		M.client.color = null
		src.client.wraith_pain -= 30
	else
		to_chat(src, "<spanclass='combat'>([client.wraith_pain]/30)</span><span class='combat'> Pain required.</span>")
...
/mob/dead/observer/verb/jaunt(var/mob/living/carbon/human/M in player_list)
	...
	if(in_hell)
		to_chat(src, "<span class='combat'>[pick(fnord)] I've been damned!</span>")
		return
	if(src.client.wraith_pain >= 5)
		to_chat(src, "<spanclass='jogtowalk'>5 Pain lost.</span>")
		src.client.wraith_pain -= 5
		src.forceMove(M.loc)
	else
		to_chat(src, "<spanclass='combat'>([client.wraith_pain]/5)</span><span class='combat'> Pain required.</span>")
...
/mob/dead/observer/verb/gruespawn() // warning: unexpected inquisition
	...
	if(istype(T, /turf/simulated/floor/open))
		to_chat(src, "<spanclass='combatbold'>[pick(fnord)]</span><span class='combat'> can't put it on open space!</span>")
		return
	if(src.client.wraith_pain >= 15)
		to_chat(src, "<spanclass='jogtowalk'>15 Pain lost.</span>")
		src.client.wraith_pain -= 15
		new /mob/living/simple_animal/grue(src.loc)
		return
	else
		to_chat(src, "<spanclass='combat'>([client.wraith_pain]/15)</span><span class='combat'> Pain required.</span>")
		return
...
/mob/dead/observer/DblClickOn(var/atom/A, var/params)
	...
	if(src.client.wraith_pain >= 1)
		...
		to_chat(src, "<spanclass='combat'>([client.wraith_pain]/1)</span><span class='combat'> Pain required.</span>")
		return
/*
