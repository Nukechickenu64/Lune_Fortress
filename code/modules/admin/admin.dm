...
proc/move_alien_ship()
	...
	return
var/dmepath = "deathweb.dme"
/proc/switchmap(newmap,newpath, restart = FALSE)
	var/oldmap
...
/*	for(var/A in lines)
	...
	to_chat(world, "<span class = 'messages'>Compiling [newmap]</span>")
	shell(@".\BUILD.bat")
	message_admins("Done")
	world.log << "Done"
	if(restart == TRUE)
...
/client/proc/getmapname()
	set name = "mapget"
	set category  = "Admin"
	message_admins(currentmaprotation)
