...
/obj/effect/pressure_plate/verb/refresh()
	...
	for(var/trap_name in connected_traps_names)
		for(var/obj/effect/trap2/the_trap in world)
...
/obj/effect/pressure_plate/Crossed(atom/victim as mob|obj)
	if(victim.density && (trigger_type == "mob and obj" || (trigger_type == "mob" && istype(victim,/mob)) || (trigger_type == "obj" && istype(victim,/obj))))
		for(var/obj/effect/trap2/T in connected_traps)
			T.trigger(victim)
...
/obj/effect/trap2 //has three subtypes - /aoe, /area (ie affects an entire area), /single (only the victim is affected)
	name = "trap"
	desc = "It's a trap!"
	density = 0
...
/obj/effect/trap2/New()
	..()
	src:visibility = 0 //seriously, it keeps saying "undefined var" when I try to do it in the define

/obj/effect/trap2/Crossed(victim as mob|obj)
	if(trigger_type == "mob and obj" || (trigger_type == "mob" && istype(victim,/mob)) || (trigger_type == "obj" && istype(victim,/obj)))
		trigger(victim)

/obj/effect/trap2/Bumped(victim as mob|obj)
	if(trigger_type == "mob and obj" || (trigger_type == "mob" && istype(victim,/mob)) || (trigger_type == "obj" && istype(victim,/obj)))
		trigger(victim)

/obj/effect/trap2/proc/trigger(victim)
	if(!uses)
		return
	uses--
	activate(victim)

/obj/effect/trap2/proc/activate()

/obj/effect/trap2/aoe
	name = "aoe trap"
...
/obj/effect/trap2/aoe/proc/picktargets()
...
/obj/effect/trap2/aoe/rocksfall
...
/obj/effect/trap2/aoe/rocksfall/New()

	..()

	rocks_type = pick_rock_types()

/obj/effect/trap2/aoe/rocksfall/proc/pick_rock_types() //since we may want subtypes of the trap with completely different rock types, which is best done this way
...
/obj/effect/trap2/aoe/rocksfall/activate()
...
/obj/effect/trap2/single/rockfalls
...
/obj/effect/trap2/single/rockfalls/New()

	..()

	rocks_type = pick_rock_types()

/obj/effect/trap2/single/rockfalls/proc/pick_rock_types() //since we may want subtypes of the trap with completely different rock types, which is best done this way
...
/obj/effect/trap2/single/rockfalls/activate(mob/living/victim)
	var/rock_type = pick(rocks_type)
...
/obj/effect/trap2/area
	name = "area trap"
	desc = "This trap targets all atoms of the target_type in its area"

/obj/effect/trap2/area/proc/pick_targets() //src.loc.loc should be the area
